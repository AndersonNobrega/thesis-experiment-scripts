...
RESULTS_BASE_DIR="$HOME/parameters_results/simple_eval"
mkdir -p "$RESULTS_BASE_DIR"

# Define the four augmentation method arguments.
# augmentation_methods=("" --synthetic_modelling --random_assign --merged)
augmentation_methods=(--merged)
...
for method in "${augmentation_methods[@]}"; do
    ...
    if [ "$method_name" == "no_args" ] ; then
        cp "$HOME/code_projects/mestrado/conf/residencial2_1.conf" "$HOME/envs/set-nialm3/configs/individual_appliances/residencial/residencial_default.conf"
    else
        cp "$HOME/code_projects/mestrado/conf/residencial2_2.conf" "$HOME/envs/set-nialm3/configs/individual_appliances/residencial/residencial_default.conf"
    fi
    ...
    if [ "$method_name" == "random_assign" ] || [ "$method_name" == "no_args" ]; then
    ...
    for run in {1..1}; do
        ...
        nialm_env
        if [ -z "$method" ]; then
            python3 "data_aug_methods_simple.py"
        else
            python3 "data_aug_methods_simple.py" "$method"
        fi
        deactivate_env
