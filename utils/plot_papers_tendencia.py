...
WEIGHT_COUNTS = {
    "Random Appliance Activation Assignement": np.array([1, 0, 0, 1, 2, 1, 3, 4, 2, 1, 0, 2, 2]),
    "Statistical Modeling/Simulation": np.array([0, 0, 1, 0, 1, 2, 1, 1, 1, 0, 0, 3, 7]),
    ...
    "Generative Adversarial Networks": np.array([0, 0, 0, 0, 0, 0, 0, 1, 1, 2, 4, 5, 4]),
...
def create_stacked_bar_plot(years, weight_counts, width, output_path):
    ...
    for label, weight_count in weight_counts.items():
        ax.bar(
            years, weight_count, width, label=label, edgecolor="black", bottom=bottom, zorder=3
        )
        bottom += weight_count
